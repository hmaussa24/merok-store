...
export function Checkout() {
  ...
  const [numeroFactura] = useState<string>(uuidv4());
  ...
  const handleGuardarCompra = async (e: React.FormEvent, metodo: string) => {
    ...
    try {
      ...
      try {
      const response =  await axios.post(`${process.env.REACT_APP_BASE_URL}/compras/with-client`, data);
      console.log('Compra guardada:', response.data);
      setRedirect(`${process.env.REACT_APP_BASE_URL}/transferencia/${id}/${response.data.id}`)
  ...
  const processWompiPayment = async (referencia: string, monto: number) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/wompi/process-payment`, {
        referencia,
        monto,
      });
  ...
  useEffect(() => {
    const fetchProducto = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/productos/${id}`);
  ...
  useEffect(() => {
    const fetchProducto = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/tallas/${talla}`);
  ...
  useEffect(() => {
    const fetchProducto = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/colors/${color}`);
      ...
      <div className="pt-16">
        ...
        <footer className="bg-black text-white text-center py-6">
          ...
          <div className="mt-2 space-x-4">
            ...
            <button onClick={() => alert('Política de privacidad')} className="hover:underline text-blue-500 bg-transparent border-none cursor-pointer">
